...
series = {}
for selected_time in ['W', 'M','Y', 'D']:
    for hospital in covid_df_filtered['ORGANIZATION_NM'].unique():
        ...
        series[(hospital,selected_time)] = series_filled
...
def update_graph(selected_time, selected_org, selected_hospital):
    ...
    for hospital in filtered_df['ORGANIZATION_NM'].unique():
        ...
        if model_cache_key in model_cache:
            ...
            model = RNNModel(model='LSTM', input_chunk_length=time_multiplier[selected_time], n_epochs=50, hidden_dim =256, n_rnn_layers=3, dropout= 0.1, random_state=42,  optimizer_kwargs={'lr': 1e-4})
