...
time_multiplier = {'ME':12,'W':52,'D':365}

min_data = {'ME':12,'W':102,'D':24}

def create_rsv_layout():
    layout = html.Div([
        html.H1("RSV Time Series Prediction", style={'textAlign': 'center'}),

        # Dropdown for Organization Name
        html.Label("Select Organization:",),
        dcc.Dropdown(id='rsv_org_filter', options=org_options, value='All', clearable=False),

        # Dropdown for Children's Hospital
        html.Label("Children's Hospital Filter:"),
        dcc.Dropdown(id='rsv_hospital_filter', options=hospital_options, value='All', clearable=False),
        
        # Dropdown for granularity
        html.Label("Timeframe Filter:"),
        dcc.Dropdown(id='rsv_time_filter', options=time_options, value='ME', clearable=False),

        # Graph
        dcc.Graph(id='rsv_admissions_graph', config={'displayModeBar': False})
    ])
    return layout

def register_rsv_callbacks(app):
    @app.callback(
        Output('rsv_admissions_graph', 'figure'),
        [Input('rsv_time_filter','value'),
            Input('rsv_org_filter', 'value'),
        Input('rsv_hospital_filter', 'value')]
    )
